...
class TrustedModule extends BaseModule{
    ...
    public function init(array $data)
    {
        ...
        foreach ($data[1] as $xuid => $datum){
            $this->trustedPeople[strval($xuid)] = $datum;
        }
    }
    ...
    public function xuidToName(string $xuid): ?string {
        if (isset($this->trustedPeople[$xuid])){
            return $this->trustedPeople[$xuid];
        }
        return null;
    }
    ...
    public function addToTrustedLimit(int $amount){
    ...
    public function addTrusted(PlayerSession $session, string $name, string $xuid): bool {
        ...
        $this->trustedPeople[strval($xuid)] = $name;
        $this->getIsland()->getPermissionModule()->addTrusted(strval($xuid));
