...
class PermissionModule extends BaseModule{
    ...
    public function getTrustedPermission(string $xuid): ?array {
        if (isset($this->trustedPermissions[$xuid])){
            return $this->trustedPermissions[$xuid];
        }
        return null;
    }

    public function addTrusted(string $xuid){
        $this->trustedPermissions[$xuid] = self::TRUSTED_AVAILABLE_PERMISSIONS;
    }
