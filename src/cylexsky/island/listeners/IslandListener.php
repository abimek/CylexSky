...
class IslandListener implements Listener{
    ...
    public function entityDamage(EntityDamageEvent $event){
        ...
        if ($event->getCause() === EntityDamageEvent::CAUSE_VOID){
            ...
            $reduce = intval(floor($wealth * .05));
            $session->getMoneyModule()->removeMoney($reduce);
            $session->sendNotification("You've lost " . TextFormat::GOLD . $reduce . Glyphs::GOLD_COIN . TextFormat::GRAY . ", 5% wealth lost!");
    ...
    public function bBreak(BlockBreakEvent $event){
        if (in_array($event->getPlayer()->getWorld()->getFolderName(), WorldManager::getWorldNames())){
            return;
        }
        $player = $event->getPlayer();
        ...
        $bord = $is->getUpgradeModule()->getIslandBorderLimit();
        $d = intval(floatval($bord/2));
        $x = $pos->getX();
        $z = $pos->getZ();
        if ($x > $d || $x < (-1 * $d)){
            $session->sendNotification("You can not break blocks beyond the island border!");
            $event->cancel();
            return;
        }
        if ($z > $d || $z < (-1 * $d)){
    ...
    public function bPlace(BlockPlaceEvent $event){
        if (in_array($event->getPlayer()->getWorld()->getFolderName(), WorldManager::getWorldNames())){
            return;
        }
        $player = $event->getPlayer();
        ...
        $z = $pos->getZ();
        if ($x > $d || $x < (-1 * $d)){
            $session->sendNotification("You can not place blocks beyond the island border!");
            $event->cancel();
            return;
        }
        if ($z > $d || $z < (-1 * $d)){
