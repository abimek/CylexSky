...
class IslandInfoUI extends SimpleForm{
    ...
    public function __construct(PlayerSession $session, bool $back = true)
    {
        ...
        $content = "";
        $content .= Glyphs::OPEN_BOOK . TextFormat::RED . "Owner: " . TextFormat::GRAY . $is->getOwnerName() . "\n";
        ...
        $content .= TextFormat::RED . "  Current Prestige: " . TextFormat::GRAY . $is->getLevelModule()->getPrestige(). "\n";
        if (!$is->getLevelModule()->getLevel() >= LevelModule::MAX_LEVEL){
            $content .= TextFormat::RED . "  Required Prestige Shards: " . TextFormat::GRAY . $is->getLevelModule()->getPrestigeShardsForNextLevel(). "\n";
